...
router.get('/', (req, res) => {
  ...
  try {
    // Ensure table exists
    try { db.exec("CREATE TABLE IF NOT EXISTS advertisements (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT NOT NULL, position TEXT, code TEXT, image TEXT, link TEXT, start_date DATE, end_date DATE, is_active INTEGER DEFAULT 1)"); } catch(e) {}
    // Get all active ads (date filtering done in JS for reliability)
    const allAdsRaw = db.prepare('SELECT * FROM advertisements WHERE is_active = 1 ORDER BY id DESC').all();
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    activeAds = allAdsRaw.filter(ad => {
      if (!ad.start_date && !ad.end_date) return true; // No date limit
      const sd = ad.start_date ? new Date(ad.start_date) : null;
      const ed = ad.end_date ? new Date(ad.end_date) : null;
      if (sd && sd > today) return false; // Not started yet
      if (ed && ed < today) return false; // Already expired
      return true;
    });
  } catch(e) { activeAds = []; }
