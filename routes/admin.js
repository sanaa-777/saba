...
router.get('/ads', requireAuth, (req, res) => {
  try {
    ...
    // Ensure table exists
    try {
      db.exec("CREATE TABLE IF NOT EXISTS advertisements (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, name TEXT NOT NULL, position TEXT, code TEXT, image TEXT, link TEXT, start_date DATE, end_date DATE, is_active INTEGER DEFAULT 1)");
    } catch(e) {}
    let ads = [];
    try {
      ads = db.prepare('SELECT * FROM advertisements ORDER BY id DESC').all();
    } catch(qErr) {
      console.error('[Ads] Query error:', qErr.message);
    }
  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        console.error('[Ads] Image error:', imgErr.message);
      }
    }

    const sd = (start_date && start_date.trim()) ? start_date.trim() : null;
    const ed = (end_date && end_date.trim()) ? end_date.trim() : null;
    const active = is_active === '1' || is_active === 'on' || is_active === true ? 1 : 0;

    db.prepare('INSERT INTO advertisements (name, position, code, image, link, start_date, end_date, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
      name || 'إعلان', position || 'header', code || '', image, link || '', sd, ed, active
    );

    return res.json({ success: true, message: 'تم إضافة الإعلان بنجاح' });
  } catch (err) {
    console.error('[Ads] Create error:', err);
    return res.status(500).json({ success: false, message: err.message || 'خطأ غير معروف' });
  }
}));
