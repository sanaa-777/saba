...
function initDatabase() {
  ...
  // Run column migration - add missing columns individually
  ...
  const extraSql = [
    "CREATE TABLE IF NOT EXISTS deleted_articles (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, original_id BIGINT, title TEXT NOT NULL, source_url TEXT, content_hash TEXT, deleted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, deleted_by TEXT DEFAULT 'admin')",
    'CREATE TABLE IF NOT EXISTS fetch_locks (lock_name TEXT PRIMARY KEY, locked_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, locked_by TEXT, expires_at TIMESTAMP NOT NULL)',
    'CREATE INDEX IF NOT EXISTS idx_news_deleted ON news(deleted_at)',
    'CREATE INDEX IF NOT EXISTS idx_news_content_hash ON news(content_hash)',
    'CREATE INDEX IF NOT EXISTS idx_news_source_url ON news(source_url)',
    'CREATE INDEX IF NOT EXISTS idx_deleted_articles_title ON deleted_articles(title)',
    ...
    'CREATE INDEX IF NOT EXISTS idx_deleted_articles_hash ON deleted_articles(content_hash)',
    'CREATE INDEX IF NOT EXISTS idx_fetch_locks_expires ON fetch_locks(expires_at)',
    `CREATE TABLE IF NOT EXISTS article_view_events (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, news_id BIGINT NOT NULL, visitor_hash TEXT NOT NULL, device_type TEXT DEFAULT 'unknown', referrer TEXT DEFAULT '', user_agent TEXT DEFAULT '', viewed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT fk_view_event_news FOREIGN KEY (news_id) REFERENCES news(id) ON DELETE CASCADE)`,
    'CREATE INDEX IF NOT EXISTS idx_view_events_time ON article_view_events(viewed_at)',
    'CREATE INDEX IF NOT EXISTS idx_view_events_news_time ON article_view_events(news_id, viewed_at)',
    'CREATE INDEX IF NOT EXISTS idx_view_events_visitor ON article_view_events(news_id, visitor_hash, viewed_at)',
  ];
  for (const sql of extraSql) {
    try { pgdb.exec(sql); } catch(e) { /* may already exist */ }
  }
  console.log('[DB] Column migration completed');

  // Fix broken sources and activate reliable ones
  try {
    ...
    const reliable = [
      ...
      ['DW Arabic', 'https://rss.dw.com/xml/rss-arb'],
    ];
    for (const [name, url] of reliable) {
      const existing = pgdb.prepare('SELECT id FROM news_sources WHERE url = ?').get(url);
      if (existing) {
        pgdb.prepare("UPDATE news_sources SET name = ?, is_active = 1, last_fetch_status = 'pending', last_error = NULL, next_fetch_at = CURRENT_TIMESTAMP WHERE id = ?").run(name, existing.id);
      } else {
        pgdb.prepare("INSERT INTO news_sources (name, url, source_type, is_active, fetch_interval, auto_publish, next_fetch_at, last_fetch_status) VALUES (?, ?, 'rss', 1, 900, 1, CURRENT_TIMESTAMP, 'pending')").run(name, url);
      }
    }
    // Deduplicate: keep only one active per URL
    const dupes = pgdb.prepare('SELECT url, MIN(id) as keep_id FROM news_sources WHERE is_active = 1 GROUP BY url HAVING COUNT(*) > 1').all();
    for (const d of dupes) {
      pgdb.prepare('UPDATE news_sources SET is_active = 0 WHERE url = ? AND id != ?').run(d.url, d.keep_id);
    }
    const activeCount = pgdb.prepare('SELECT COUNT(*) as cnt FROM news_sources WHERE is_active = 1').get();
    console.log(`[DB] Active sources: ${activeCount.cnt}`);
  } catch(e) { console.log('[DB] Source fix error:', e.message); }
